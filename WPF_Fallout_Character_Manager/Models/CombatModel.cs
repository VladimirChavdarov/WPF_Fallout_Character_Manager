...
{
    class CombatModel : ModelBase, ISerializable<CombatModelDTO>
    {
        // constructor
        public CombatModel()
        {
        ...
        // helpers
        public CombatModelDTO ToDto()
        {
            return new CombatModelDTO
            {
                ActionPoints = ActionPoints.ToDto(),
                MaxStaminaPoints = MaxStaminaPoints.ToDto(),
                StaminaPoints = StaminaPoints.ToDto(),
                MaxHealthPoints = MaxHealthPoints.ToDto(),
                HealthPoints = HealthPoints.ToDto(),
                ArmorClass = ArmorClass.ToDto(),
                DamageThreshold = DamageThreshold.ToDto(),
                CombatSequence = CombatSequence.ToDto(),
                HealingRate = HealingRate.ToDto(),
                Fatigue = Fatigue.ToDto(),
            };
        }

        public void FromDto(CombatModelDTO dto, bool versionMismatch = false)
        {
            ActionPoints = new ModInt(dto.ActionPoints);
            MaxStaminaPoints = new ModInt(dto.MaxStaminaPoints);
            StaminaPoints = new ModInt(dto.StaminaPoints);
            MaxHealthPoints = new ModInt(dto.MaxHealthPoints);
            HealthPoints = new ModInt(dto.HealthPoints);
            ArmorClass = new ModInt(dto.ArmorClass);
            DamageThreshold = new ModInt(dto.DamageThreshold);
            CombatSequence = new ModInt(dto.CombatSequence);
            HealingRate = new ModInt(dto.HealingRate);
            Fatigue = new ModInt(dto.Fatigue);
        }
